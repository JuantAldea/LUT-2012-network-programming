...
unsigned int print_packed_addresses(int count, struct sockaddr_storage* addrs)
{
    ...
    while(i < count){
        ...
        slen = (temp->ss_family == AF_INET ? sizeof(struct sockaddr_in) : temp->ss_family == AF_INET6 ? sizeof(struct sockaddr_in6) : 0);
        if (slen > 0) {
            memset(&hostname,0,NI_MAXHOST);
            // Get the numeric representation of hostname into buffer
            if (getnameinfo((struct sockaddr*)temp,slen,hostname,NI_MAXHOST,NULL,0,NI_NUMERICHOST) < 0)
                perror("getnameinfo");
            else
                printf("\t[%s] %s\n",(temp->ss_family == PF_INET ? "IPv4" : "IPv6"),hostname);
